    ...
    var dlTvNamesAndParseToLocal = function (fileName) {
        ...
        return new WinJS.Promise(function (completex) {
            if (fileName == "FirstDay") {
                ...
                localFolder.createFileAsync(fileName + ".txt", Windows.Storage.CreationCollisionOption.replaceExisting).then(
                    function (file) {
                        new WinJS.Promise(function (downloadedResponseText) {
                            if (fileName == "TvNames") {
                            ...
                            function (responseText) {
                                ...
                                localSettings.values["connection"] = false;
                                if (responseText) {
                                    saveToFile(file, responseText);
                                    var programs = JSON.parse(responseText);
                                    completex(programs);
                                } else {
                                    Windows.UI.Popups.MessageDialog("Не е налично съдържание. Проверете връзката си с интернет.", "Грешка при свързване").showAsync();
                                }
                            },
                            function (messager) {
                                error(messager);
